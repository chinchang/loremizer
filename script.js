window.loremiscous =
  ...
  (function() {
    ...
    function replaceText({ textLengthMultiplier }) {
      ...
      nodes.forEach(n => {
        ...
        n.nodeValue = n.nodeValue
          ...
          .split(/(\d+|[^\w\s]|_|\s)/)
          .map(str => {
            const length = str.length * textLengthMultiplier;
            // If this is just numbers
            if (parseInt(str, 10)) {
              return parseInt(
                Math.random()
                  .toString()
                  .slice(2, Math.min(length + 2, 18)),
                10
              );
            } else if (
              str.length === 1 &&
    ...
    function replaceTextWithScribble() {
    ...
    function runLoremiscous(options) {
      if (operations.length) {
        ...
        if (options["replaceText"]) {
          requestIdleCallback(() => {
            replaceText({
              textLengthMultiplier: options["textLengthMultiplier"]
            });
          });
        }
        if (options["replaceTextWithScribble"]) {
          requestIdleCallback(replaceTextWithScribble);
        }
        if (options["replaceImage"]) {
          requestIdleCallback(replaceImgs);
        }
        if (options["replaceBgImage"]) {
          requestIdleCallback(replaceBgImgs);
        }
        if (options["replaceIframe"]) {
